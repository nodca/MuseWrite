...
from app.schemas.chat import (
    ...
    InlineSuggestionStreamEvent,
    InlineSuggestionStreamRequest,
)
...
from .chat_helpers import (
    build_rewrite_user_input as _build_rewrite_user_input,
    ensure_project_scope_access as _ensure_project_access,
    normalize_generated_text as _normalize_generated_text,
)
...
def _resolve_temperature(*, profile: str | None, override: float | None) -> float:
    ...
    token = str(profile or "suggestion").strip().lower()
    if token == "action":
    ...
    if token == "suggestion":
        return _clamp_temperature(float(settings.llm_temperature_suggestion))
    return _clamp_temperature(float(settings.llm_temperature))


async def _ws_send_event(websocket: WebSocket, event: InlineSuggestionStreamEvent) -> None:
    await websocket.send_json(event.model_dump())


async def _ws_send_error(websocket: WebSocket, message: str, *, close_code: int = 1011) -> None:
    await _ws_send_event(websocket, InlineSuggestionStreamEvent(type="error", message=message))
    await websocket.close(code=close_code)


@router.websocket("/inline-suggestions")
async def inline_suggestion_stream(
    websocket: WebSocket,
):
    ...
    try:
        payload = InlineSuggestionStreamRequest.model_validate_json(raw)
    except Exception:
    ...
    await _ws_send_event(websocket, InlineSuggestionStreamEvent(type="start", text=""))
    ...
    try:
        ...
        await _ws_send_error(websocket, f"inline suggestion context failed: {exc}")
        return
    ...
    if provider not in {"openai_compatible", "deepseek"}:
        await _ws_send_error(
            websocket,
            f"inline suggestion ws currently supports openai_compatible/deepseek only (provider={provider})",
            close_code=1003,
        )
        return
    ...
    try:
        ...
        await _ws_send_error(websocket, f"inline suggestion generation failed: {exc}")
        return

    content = str(data.get("choices", [{}])[0].get("message", {}).get("content", "") or "")
    suggestion = _normalize_generated_text(content, max_length=2600)
    if not suggestion:
    ...
    usage = {
        ...
        "model": model,
        "temperature": temperature,
        "temperature_profile": payload.temperature_profile or "suggestion",
    ...
    try:
        ...
        for idx in range(0, text_length, chunk_size):
            ...
            await _ws_send_event(websocket, InlineSuggestionStreamEvent(type="delta", text=delta))
            if idx + chunk_size < text_length:
                await asyncio.sleep(0.02)
        await _ws_send_event(
            websocket,
            InlineSuggestionStreamEvent(type="done", text=suggestion, usage=usage),
        )
    except WebSocketDisconnect:
        return
    except Exception as exc:
        await _ws_send_error(websocket, f"inline suggestion stream failed: {exc}")
        return

    await websocket.close()


@router.post("/rewrite/polish", response_model=SelectionRewriteResponse)
async def generate_selection_polish(
    payload: SelectionRewriteRequest,
    db: Session = Depends(get_session),
    principal: AuthPrincipal = Depends(get_current_principal),
):
    return await _generate_selection_rewrite("polish", payload, db, principal)


@router.post("/rewrite/expand", response_model=SelectionRewriteResponse)
async def generate_selection_expand(
    payload: SelectionRewriteRequest,
    db: Session = Depends(get_session),
    principal: AuthPrincipal = Depends(get_current_principal),
):
    return await _generate_selection_rewrite("expand", payload, db, principal)


async def _generate_selection_rewrite(
    ...
    # ── Book Memory OS injection for rewrite ──
    if settings.use_memory_pipeline and payload.chapter_id:
        try:
            ...
            chapter_index = int(getattr(chapter_for_context, "chapter_index", 0) or 0) if chapter_for_context else None
            memory_ctx = build_rewrite_memory_context(
                db,
                project_id=payload.project_id,
            ...
            memory_hint = memory_ctx.to_prompt_text()
            if memory_hint:
                outline_hint_parts.insert(0, memory_hint)
        except Exception:
    ...
    try:
        ...
        raise HTTPException(status_code=502, detail=f"selection rewrite generation failed: {exc}")

    suggestion = _normalize_generated_text(
