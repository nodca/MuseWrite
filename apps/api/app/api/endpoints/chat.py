...
def _enforce_quality_gate(generation: ChatGenerationResult, model_context: dict) -> ChatGenerationResult:
...
def _build_action_provenance(model_context: dict[str, Any] | None) -> dict[str, Any]:
...
def _action_provenance_from_payload(action: Any) -> dict[str, Any]:
...
def _ensure_action_access(db: Session, action_id: int, principal: AuthPrincipal):
    action = get_action_by_id(db, action_id)
    if not action:
        raise HTTPException(status_code=404, detail="action not found")
    _ensure_session_access(db, int(action.session_id), principal)
    return action


def _ensure_project_access(project_id: int, principal: AuthPrincipal) -> None:
    ensure_project_access(principal.user_id, int(project_id))


def _normalize_index_lifecycle_dead_letter_row(
    row: Any,
    *,
...
def _build_ghost_user_input(prefix_text: str, *, style_prefix: str = "", outline_hint: str = "") -> str:
...
def _normalize_ghost_suggestion(value: str) -> str:
    text = (value or "").strip()
    if not text:
        return ""
